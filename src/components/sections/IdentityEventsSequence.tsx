...
export default function IdentityEventsSequence() {
  ...
  useMotionValueEvent(scrollYProgress, "change", (latest) => {
    // 1. CONTENT INDEXING
    // 0.0 - 0.40 handles the split and the collapse
    // 0.40 - 1.0 handles the 5 events
    ...
    // 2. CHROME THEME ORCHESTRATION
    // Doors Phase (0.0 to 0.4): Dark Chrome (Black Icons), Hide Logo after split starts
    if (latest < 0.15) {
      ...
      // Events Gallery Phase (0.4+): Light Chrome (White Icons), Show Logo, but Dark Cart
  ...
  // Doors Animation (From center 0 to full 50vw translation)
  const doorOffset = useTransform(scrollYProgress, (p) => {
    const t = clamp(p / splitEnd, 0, 1);
    return `${easeInOut(t) * 50}vw`; // moves out 50vw
  });

  // Media Width Collapse
  ...
  // ── Migrating "UPCOMING EVENTS" Kicker ────────────────────────────────
  // Starts at the bottom left. Once collapse starts, moves up to the 40px spot.
  const labelTop = useTransform(scrollYProgress, (p) => {
    const t = norm(p, collapseStart, collapseEnd);
    return `calc(85vh + (89px - 85vh) * ${t})`; // Aligned to center-line of See All Events row (88px + 9px)
  });

  const labelLeft = useTransform(scrollYProgress, (p) => {
    const t = norm(p, collapseStart, collapseEnd);
    return `calc(50% + ((72vw + 40px) - 50%) * ${t})`; // Starts at absolute center (50%)
  });

  const labelTransX = useTransform(scrollYProgress, (p) => {
    const t = norm(p, collapseStart, collapseEnd);
    return `${(1 - t) * -50}%`; // Starts at -50% for center-anchor, migrates to 0% for left-anchor
  });
  ...
  // Fade from white #FFFFFF over dark cinematic -> #9C8981 in white side panel
    ...
    <section ref={sectionRef} style={{ height: "800vh", position: "relative" }} aria-label="TORQ Identity & Upcoming Events Sequence">
      <div style={{ position: "sticky", top: 0, height: "100vh", overflow: "hidden", backgroundColor: "#000" }}>
        ...
        <motion.div style={{ position: "absolute", top: 0, left: 0, bottom: 0, width: mediaWidth, overflow: "hidden", zIndex: 10 }}>
          ...
          {/* Core darkening gradient for text legibility, strictly bound to media height */}
          <div
            style={{
              position: "absolute",
              inset: 0,
          ...
          <motion.div
            style={{
              position: "absolute",
              bottom: "7%",
              right: "40px", // Aligned with 40px discipline
              display: "flex",
              flexDirection: "column",
              gap: "10px",
              alignItems: "center",
              zIndex: 25, // Above side panel header logic
        ...
        <motion.div
          style={{
            ...
            left: labelLeft,
            x: labelTransX,
            zIndex: 30, // Above side panel but below doors!
            pointerEvents: "none",
          }}
        >
          <motion.span
          ...
          >
            <a
              ...
              style={{
                ...
                whiteSpace: "nowrap",
                lineHeight: 1, // Strict line height for alignment
              }}
            >
              SEE ALL EVENTS
            </a>
